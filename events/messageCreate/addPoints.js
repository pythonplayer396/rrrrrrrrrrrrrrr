...
module.exports = async (message) => {
  ...
  const addPoints = Number(args[1]);
  // If they didn't provide a user, return
  if (!addPoints) {
    return message.reply("Please Provide A Valid Number To Be Added");
  }
  ...
  if (!staff) {
    const newStaff = await new staffPoints({
      staffId: staffId,
      points: addPoints,
    });
    await newStaff.save();
    const staffPEmbed = new EmbedBuilder()
      ...
      .setDescription(`Current Points Of <@${newStaff.staffId}>`) // ✅ Mention works here!
      .addFields({
        name: "Points:",
        value: `${newStaff.points}`, // ✅ Make sure it's a string!
      })
      .setColor("Random");
  ...
  try {
    staff.points = staff.points + addPoints;
    staff.save();
  } catch (error) {
    console.log(error);
    message.reply("An Error Occurred While Adding Points.");
  }
  const staffPEmbed = new EmbedBuilder()
    .setTitle(`Points Of Staff:`)
